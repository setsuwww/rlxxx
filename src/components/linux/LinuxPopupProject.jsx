    ...
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 flex items-center justify-center z-[1000] bg-black/50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            <div className="flex items-center justify-between bg-gradient-to-b from-zinc-200/50 to-zinc-200/70 border-zinc-300 inset-shadow-sm inset-shadow-zinc-200 px-4 py-2 border-b">
              ...
              <span className="bg-zinc-200 px-4 py-0.5 md:px-6 md:py-1 rounded-lg border border-zinc-400/20 font-semibold text-left text-xs md:text-sm text-zinc-700 truncate">
                {project.title}
              </span>
              <div className="flex items-center space-x-1">
                <button onClick={onPrev} className="p-0.5 md:p-1 rounded-md bg-white shadow-xs border border-gray-200 hover:bg-zinc-300 transition">
                  <ChevronLeft className="w-4 h-4 md:w-5 md:h-5 text-zinc-700" />
                </button>
                <button onClick={onNext} className="p-0.5 md:p-1 rounded-md bg-white shadow-xs border border-gray-200 hover:bg-zinc-300 transition">
